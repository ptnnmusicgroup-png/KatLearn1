...
(function(){
  ...
  function boot(){window.addEventListener('8b1-auth-change',()=>setTimeout(load,80));if(window.studyStore?.user)setTimeout(load,80);document.addEventListener('click',e=>{const answer=e.target.closest?.('.answer');if(answer&&answer.dataset.right==='true'&&ready)setTimeout(()=>{cloudCoins+=10;persist()},1000)},true);document.addEventListener('click',e=>{const btn=e.target.closest?.('.shop-grid button');if(!btn||!ready)return;const price=Number(btn.dataset.price||0);if(!price||cloudCoins<price)return;setTimeout(()=>{cloudCoins=Math.max(0,cloudCoins-price);persist()},500)},true)}
