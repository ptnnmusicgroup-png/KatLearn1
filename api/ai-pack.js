const{requireUser,rateLimit,safetyIdentifier,generateGemini,send,method}=require("./_kat-ai");
...
if(words.length<wordCount){
 ...
 result={...result,pack:retryResult.pack||result.pack||{}};
}
if(words.length<wordCount)throw Object.assign(new Error("Gemini chưa tạo đủ "+wordCount+" từ. Hãy thử lại với topic cụ thể hơn."),{status:502});
words=words.slice(0,wordCount);
result.pack=result.pack||{};result.pack.topic=prompt;result.pack.difficulty=difficulty;result.pack.purpose=purpose;result.words=words;return send(res,200,result)}catch(error){return send(res,error.status||500,{error:"Kat AI không thể tạo bộ từ: "+String(error.message||error)},error.retryAfter?{"Retry-After":String(error.retryAfter)}:{})}};