...
function publicScore(coins,xp){return{coins:Math.max(0,Number(coins)||0),xp:Math.max(0,Number(xp)||0),updatedAt:FieldValue.serverTimestamp()}}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const userRef=db.doc("users/"+token.uid),leaderboardRef=db.doc("leaderboard/"+token.uid);
    ...
    if(action==="answer"){
      ...
      const result=await db.runTransaction(async transaction=>{
        ...
        transaction.set(leaderboardRef,publicScore(coins,xp),{merge:true});
    ...
    if(action==="purchase"){
      ...
      const result=await db.runTransaction(async transaction=>{
        ...
        transaction.set(leaderboardRef,publicScore(coins-price,xp),{merge:true});
