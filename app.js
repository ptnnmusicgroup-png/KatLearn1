...
const savedFirebase=localStorage.getItem('8b1-firebase-config'),defaultFirebase=window.KATLEARN_FIREBASE_CONFIG;
const settingsModal=$('#settingsModal'),settingsContent=$('#settingsContent');
if(settingsContent&&!settingsContent.dataset.ready){
  settingsContent.dataset.ready='1';
  settingsContent.innerHTML='<h2>⚙ Cấu hình Firebase & AI</h2><p class="subtext">Firebase Web config có thể hiển thị trong trình duyệt. Không dán GEMINI_API_KEY vào đây — Kat AI dùng khóa bí mật ở máy chủ.</p><form id="settingsForm" class="teacher-form"><label>Firebase Web App Config (JSON)<textarea id="firebaseConfig" rows="11" spellcheck="false"></textarea></label><div style="display:flex;gap:8px;justify-content:flex-end"><button type="button" class="teacher-btn secondary" id="settingsCancel">Hủy</button><button type="submit" class="primary-btn">Lưu & kết nối</button></div></form>';
}
let firebaseConfig=defaultFirebase;
try{const raw=localStorage.getItem('8b1-firebase-config');if(raw)firebaseConfig=JSON.parse(raw)}catch(_){localStorage.removeItem('8b1-firebase-config')}
const firebaseConfigBox=$('#firebaseConfig');if(firebaseConfigBox)firebaseConfigBox.value=JSON.stringify(firebaseConfig,null,2);
window.studyStore.connect(firebaseConfig).then(()=>syncProfile()).catch(e=>console.warn('Firebase chưa kết nối:',e));
const settingsBtn=$('#settingsBtn'),settingsForm=$('#settingsForm');
...
if($('#settingsCancel'))$('#settingsCancel').onclick=()=>settingsModal?.classList.remove('show');
