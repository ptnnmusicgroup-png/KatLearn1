...
function renderVocabularyViews(filter=''){const q=filter.trim().toLowerCase(),visible=vocab.filter(v=>{const word=String(v?.word||''),mean=String(v?.mean||v?.meaning_vi||'');return !q||word.toLowerCase().includes(q)||mean.toLowerCase().includes(q)}),total=vocab.length,done=Math.min(knownWordKeys.size,total),progress=total?Math.round(done/total*100):0;$('#wordTotalStat').textContent=total;$('#wordKnownStat').textContent=done;$('#wordUnknownStat').textContent=Math.max(0,total-done);$('#wordProgressStat').textContent=progress+'%';$('#personalWordCount').textContent=total;$('#personalKnownCount').textContent=done;$('#personalPackCount').textContent=total?1:0;$('#wordTable').innerHTML=visible.length?visible.map((v,i)=>`<div class="word-table-row"><span><b>${esc(v.word)}</b><small>${i+1}/${total}</small></span><span>${esc(v.mean)}</span><span>${esc(v.pron||'—')}</span><span><i class="word-status ${knownWordKeys.has(vocabKey(v))?'':'new'}">${knownWordKeys.has(vocabKey(v))?'Đã thuộc':'Mới'}</i></span></div>`).join(''):'<div class="empty-state">Chưa có từ phù hợp.</div>'}
