...
let coins=0,energy=0,cardIndex=0,known=0,question=1,sessionCoins=0,mode='engvi',answered=false,knownWordKeys=new Set();
...
function vocabKey(v){return String(v?.word||'').trim().toLowerCase()+'::'+String(v?.mean||v?.meaning_vi||'').trim().toLowerCase()}
...
function renderVocabularyViews(filter=''){const q=filter.trim().toLowerCase(),visible=vocab.filter(v=>{const word=String(v?.word||''),mean=String(v?.mean||v?.meaning_vi||'');return !q||word.toLowerCase().includes(q)||mean.toLowerCase().includes(q)}),total=vocab.length,done=Math.min(knownWordKeys.size,total),progress=total?Math.round(done/total*100):0;$('#wordTotalStat').textContent=total;$('#wordKnownStat').textContent=done;$('#wordUnknownStat').textContent=Math.max(0,total-done);$('#wordProgressStat').textContent=progress+'%';$('#personalWordCount').textContent=total;$('#personalKnownCount').textContent=done;$('#personalPackCount').textContent=total?1:0;$('#wordTable').innerHTML=visible.length?visible.map((v,i)=>`<div class="word-table-row"><span><b>${esc(v.word)}</b><small>${i+1}/${total}</small></span><span>${esc(v.mean)}</span><span>${esc(v.pron||'—')}</span><span><i class="word-status ${knownWordKeys.has(vocabKey(v))?'':'new'}">${i<done?'Đã thuộc':'Mới'}</i></span></div>`).join(''):'<div class="empty-state">Chưa có từ phù hợp.</div>'}
...
window.addEventListener('katlearn-personal-pack-open',e=>{const words=Array.isArray(e.detail?.words)?e.detail.words:[];vocab=words;setVocabSource({kind:'personal',id:String(e.detail?.id||'')});cardIndex=0;localStorage.setItem('katlearn-vocab',JSON.stringify(vocab));renderCard();renderQuiz();});
