...
async function answer(btn,correct){if(answered)return;answered=true;const good=btn.dataset.right==='true',v=vocab[(question-1)%vocab.length];$$('.answer').forEach(b=>{if(b.dataset.right==='true')b.classList.add('correct')});if(good){let awarded=location.hostname==='localhost'||location.hostname==='127.0.0.1'||!window.studyStore?.user;let rewardError='';if(window.studyStore?.user){try{await syncProfile();const token=await window.studyStore.getIdToken();const res=await fetch(gameEndpoint(),{method:'POST',headers:{'Content-Type':'application/json',...(token?{Authorization:'Bearer '+token}:{})},body:JSON.stringify({action:'answer',word:v.word,meaning:v.mean,correct:true})});const data=await res.json().catch(()=>({}));awarded=res.ok&&data.ok;if(!awarded)rewardError=data.error||'Máy chủ chưa xác nhận được phần thưởng.'}catch(error){awarded=false;rewardError=error?.message||'Không thể kết nối máy chủ phần thưởng.'}}if(awarded){sessionCoins+=10;coins+=10;energy+=10;updateCoins();$('#sessionCoins').textContent=sessionCoins;$('#feedback').textContent='Chính xác! +10 xu 🪙'}else{$('#feedback').textContent='Chính xác! KatCoin chưa được cộng: '+rewardError}}else{btn.classList.add('wrong');$('#feedback').textContent='Đáp án đúng là: '+correct;$('#feedback').style.color='#e36b63'}window.studyStore?.recordAnswer({word:v.word,meaning:v.mean,mode,correct:good,reviewDueAt:Date.now()+30*24*60*60*1000});setTimeout(()=>{if(question>=10){$('#reward').textContent='+'+sessionCoins+' 🪙';$('#resultModal').classList.add('show');question=1;sessionCoins=0;$('#sessionCoins').textContent='0'}else{question++;renderQuiz()}},900)}
