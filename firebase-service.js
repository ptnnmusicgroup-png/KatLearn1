...
(function(){
  ...
  window.studyStore={
    ...
    async saveProfile(data){if(!db||!currentUser)return;const user=currentUser,payload={...data};['__coinsAuthoritative','coins','energy','streak','lastStudyDay','dailyQuestions','dailyCorrect','questionsAnswered','correctAnswers','ownedThemes','teacherUid'].forEach(k=>delete payload[k]);return api.setDoc(api.doc(db,'users',this.userId),{displayName:user.displayName||user.email?.split('@')[0]||'KatLearn Student',email:user.email||'',photoURL:user.photoURL||'',updatedAt:api.serverTimestamp(),...payload},{merge:true})},
