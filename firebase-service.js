...
(function(){
  ...
  function renderAccountUi(user){
    const loginBtn=document.querySelector('#loginBtn'),trigger=document.querySelector('#accountTrigger');
    ...
    if(!loginBtn||!trigger)return;
    if(!user){loginBtn.hidden=false;trigger.hidden=true;return}
    ...
    if(trigger.dataset.authBound!=='1'){trigger.dataset.authBound='1';trigger.onclick=()=>{const panel=document.querySelector('#accountPanel');if(panel)panel.hidden=!panel.hidden}};
    const logout=document.querySelector('#logoutBtn');if(logout&&logout.dataset.authBound!=='1'){logout.dataset.authBound='1';logout.onclick=async()=>{await window.studyStore.signOut();location.reload()}};
  }
  window.studyStore={
    ...
    async signInCustomToken(token){if(!auth)throw new Error('Hãy kết nối Firebase trước.');const {signInWithCustomToken}=await import('https://www.gstatic.com/firebasejs/10.14.1/firebase-auth.js');const result=await signInWithCustomToken(auth,token);currentUser=result.user;renderAccountUi(currentUser);return currentUser},
    async signIn(providerName){
      ...
      currentUser=result.user;renderAccountUi(currentUser);
    ...
    async signOut(){if(auth)await api.auth.signOut(auth);currentUser=null;renderAccountUi(null)},
    async saveProfile(data){if(!db||!currentUser)return;const user=currentUser,payload={...data},allowCoins=payload.__coinsAuthoritative===true;delete payload.__coinsAuthoritative;if(!allowCoins)delete payload.coins;return api.setDoc(api.doc(db,'users',this.userId),{displayName:user.displayName||user.email?.split('@')[0]||'KatLearn Student',email:user.email||'',photoURL:user.photoURL||'',updatedAt:api.serverTimestamp(),...payload},{merge:true})},
