...
(function(){
  ...
  window.studyStore={
    ...
    async myClasses(){if(!db||!currentUser)return[];const profile=await this.loadProfile();const ids=Array.isArray(profile?.joinedClassIds)?profile.joinedClassIds:[];const rows=await Promise.all(ids.slice(0,20).map(async id=>{const snap=await api.getDoc(api.doc(db,'classes',id));return snap.exists()?{id:snap.id,...snap.data()}:null}));return rows.filter(Boolean)},
    async joinClass(code){if(!db||!currentUser)throw new Error('Hãy đăng nhập trước.');const clean=String(code||'').trim().toUpperCase();if(!/^[A-Z0-9]{6}$/.test(clean))throw new Error('Mã lớp phải gồm 6 ký tự.');const invite=await api.getDoc(api.doc(db,'classInvites',clean));if(!invite.exists()||invite.data()?.active===false)throw new Error('Mã lớp không tồn tại hoặc đã bị khóa.');const info=invite.data();const cls=await api.getDoc(api.doc(db,'classes',info.classId));if(!cls.exists())throw new Error('Lớp học không còn tồn tại.');await api.setDoc(api.doc(db,'classes',info.classId,'members',currentUser.uid),{uid:currentUser.uid,email:currentUser.email||'',displayName:currentUser.displayName||currentUser.email?.split('@')[0]||'KatLearn Student',joinedAt:api.serverTimestamp(),joinCode:clean,source:'student'});const profile=await this.loadProfile();const ids=Array.isArray(profile?.joinedClassIds)?profile.joinedClassIds:[];if(!ids.includes(info.classId))await this.saveProfile({joinedClassIds:[...ids,info.classId]});return{id:cls.id,...cls.data()};},
    async leaveClass(classId){if(!db||!currentUser)return;await api.deleteDoc(api.doc(db,'classes',classId,'members',currentUser.uid));const profile=await this.loadProfile();const ids=(Array.isArray(profile?.joinedClassIds)?profile.joinedClassIds:[]).filter(id=>id!==classId);await this.saveProfile({joinedClassIds:ids});},
