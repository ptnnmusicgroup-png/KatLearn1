...
(function(){
  ...
  function installManualAI(){
    ...
    const fillFromAI=async(value,silent=false)=>{
      ...
      try{const res=await fetch('/api/vocab-assist',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({word:clean})});const data=await res.json();if(!res.ok)throw new Error(data.error||'AI chưa sẵn sàng');if(id!==requestId||word.value.trim()!==clean)return;meaning.value=data.meaning||meaning.value;pron.value=data.pronunciation||pron.value;meaning.dispatchEvent(new Event('input',{bubbles:true}));pron.dispatchEvent(new Event('input',{bubbles:true}));if(!silent)toast(`✓ AI đã điền thông tin cho “${clean}”.`)}catch(err){if(!silent)toast(`❌ ${err.message}`)}finally{if(!silent){btn.disabled=false;btn.textContent='✨ AI tự điền nghĩa & phiên âm'}}
    };
