...
app.post('/api/context-example',async(req,res)=>{
  ...
  }catch(error){res.status(error.status||500).json({error:'Không tạo được câu ngữ cảnh AI: '+error.message})}
});
...
app.post('/api/vocab-assist',async(req,res)=>{
  ...
  try{
    const result=await jsonAI('Bạn là từ điển Anh–Việt dành cho học sinh lớp 8. Chỉ trả lời JSON hợp lệ có đúng hai khóa: meaning (nghĩa tiếng Việt ngắn gọn) và pronunciation (phiên âm IPA Anh-Anh đặt giữa dấu /). Không thêm markdown hay giải thích.',`Tra từ tiếng Anh: ${word}`);
    res.json({meaning:String(result.meaning||''),pronunciation:String(result.pronunciation||'')});
  }catch(error){res.status(error.status||500).json({error:'Kat AI không thể xử lý từ này: '+error.message})}
});
...
app.post('/api/ai-pack',async(req,res)=>{
  ...
  try{
    ...
    result.pack=result.pack||{}; result.pack.topic=topic; result.pack.difficulty=difficulty; result.pack.purpose=purpose;
    result.words=Array.isArray(result.words)?result.words.slice(0,wordCount):[];
    res.json(result);
  }catch(error){res.status(error.status||500).json({error:'Không tạo được pack AI: '+error.message})}
});
...
app.post('/api/ai-regenerate-word',async(req,res)=>{
  ...
  try{
    ...
    res.json(item);
  }catch(error){res.status(error.status||500).json({error:'Không tạo lại được từ: '+error.message})}
});
